...
// ─────────────────────────────────────────────────────────────────────────────
// NODE REGISTRY
// Every node maps to exactly one Temporal worker activity.
// Structure: Nodeid, type, category, label, inputs, outputs, info,
//            configValues (defaults), _schema (config panel UI definition)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// API ROUTE — Returns all nodes or filtered by category
// Usage:
//   GET /api/nodes            → all nodes
//   GET /api/nodes?category=AI         → AI nodes only
//   GET /api/nodes?category=Industrial → IIoT nodes only
// ─────────────────────────────────────────────────────────────────────────────
...
export async function GET(request: Request) {
  ...
  const grouped = nodes.reduce((acc, node) => {
    ...
    acc[node.category].nodes.push(node);
    return acc;
  }, {} as Record<string, { category: string; categoryLabel: string; nodes: typeof nodes }>);
